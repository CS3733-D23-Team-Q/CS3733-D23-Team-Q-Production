...
public class PatientTransportRequestDaoImpl implements GenDao<PatientTransportRequest, Integer> {
    ...
    public boolean deleteRow(Integer requestID) {
        try (Connection connection = GenDao.connect(); PreparedStatement st = connection.prepareStatement("DELETE FROM \"patientTransportRequests\" WHERE \"requestID\" = ?")) {
            ;
    ...
    public boolean addRow(PatientTransportRequest request) {
        try (Connection conn = GenDao.connect(); PreparedStatement stmt = conn.prepareStatement("INSERT INTO \"patientTransportRequest\"(requester, progress, assignee, \"specialInstructions\", \"transport\", \"roomNum\") VALUES (?, ?, ?, ?, ?, ?)")) {
    ...
    public boolean populate() {
        try {
            ...
            while (rst.next()) {
                patientTransportRequests.add(new PatientTransportRequest(rst.getInt("requestID"), rst.getString("requester"), rst.getInt("progress"), rst.getString("assignee"), nodeTable.retrieveRow(rst.getInt("node")), rst.getString("specialInstructions"), rst.getString("item")));
            }
            conn.close();
            stm.close();
        } catch (Exception e) {
