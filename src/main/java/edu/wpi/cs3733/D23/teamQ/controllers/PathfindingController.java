...
public class PathfindingController {
    ...
    @FXML
    RadioMenuItem aStarSelect;
    @FXML
    RadioMenuItem bfsSelect;
    @FXML
    RadioMenuItem dfsSelect;
    @FXML
    RadioMenuItem djikstraSelect;
    @FXML
    Menu dateMenu;
    @FXML
    TextArea textualPathfinding;
    @FXML
            ...
            for (int i = 0; i < cfnodes.size(); i++) {
                if (cfnodes.get(i).getKey() == nodeid) {
                    ...
                    highlightedNodesp.add(
                            0, Triple.of(node, floor, 0)); // int index = parent.getChildren().indexOf(node);
                }
            }
        }
    }
                ...
                if (cfnodes.get(i).getKey() == nodeid) {
                    ...
                    node.setStyle(
                            "-fx-background-color: lightblue;"
                                    ...
                                    + "-fx-background-insets: 0px;");
                    highlightedNodesp.add(
                            1, Triple.of(node, floor, 0)); // int index = parent.getChildren().indexOf(node);
                }
            }
        }
    }
