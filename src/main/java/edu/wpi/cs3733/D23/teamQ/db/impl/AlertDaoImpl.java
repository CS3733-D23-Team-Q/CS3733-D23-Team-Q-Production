...
public class AlertDaoImpl {
  ...
  public boolean deleteRow(Integer alertID) {
    try (Connection conn = GenDao.connect();
        PreparedStatement stmt = conn.prepareStatement("DELETE FROM alert WHERE \"alertID\" = ?")) {
  ...
  public boolean populate() {
    try {
      ...
      while (rs.next()) {
        Alert a = new Alert(rs.getInt("alertID"), rs.getLong("timestamp"), rs.getString("message"));
        alerts.add(a);
      }
      con.close();
      st.close();
