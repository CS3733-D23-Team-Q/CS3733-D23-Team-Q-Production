...
public class MedicalSuppliesRequestDaoImpl implements GenDao<MedicalSuppliesRequest, Integer> {
    private List<MedicalSuppliesRequest> medicalSuppliesRequests = new ArrayList<MedicalSuppliesRequest>();
    ...
    public boolean updateRow(Integer requestID, MedicalSuppliesRequest newRequest) {
        try (Connection connection = GenDao.connect(); PreparedStatement st = connection.prepareStatement("UPDATE \"medicalSuppliesRequest\" SET \"requestID\" = ?, requester = ?, progress = ?, assignee = ?, \"nodeID\" = ?, item = ?, quantity = ?, \"specialInstructions\" = ? " + "WHERE \"requestID\" = ?")) {
    ...
    public boolean deleteRow(Integer requestID) {
        try (Connection connection = GenDao.connect(); PreparedStatement st = connection.prepareStatement("DELETE FROM \"medicalSuppliesRequest\" WHERE \"requestID\" = ?")) {
            ;
            st.setInt(1, requestID);
            st.executeUpdate();
        } catch (SQLException e) {
    ...
    public boolean addRow(MedicalSuppliesRequest request) {
        try (Connection conn = GenDao.connect(); PreparedStatement stmt = conn.prepareStatement("INSERT INTO \"medicalSuppliesRequest\"(requester, progress, assignee, \"nodeID\", \"specialInstructions\", \"date\", \"time\", \"item\", \"quantity\") VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
    ...
    public boolean populate() {
        try {
            ...
            while (rst.next()) {
                medicalSuppliesRequests.add(new MedicalSuppliesRequest(rst.getInt("requestID"), rst.getString("requester"), rst.getInt("progress"), rst.getString("assignee"), nodeTable.retrieveRow(rst.getInt("nodeID")), rst.getString("specialInstructions"), rst.getDate("date"), rst.getString("time"), rst.getString("item"), rst.getInt("quantity")));
            }
            conn.close();
            stm.close();
        } catch (Exception e) {
