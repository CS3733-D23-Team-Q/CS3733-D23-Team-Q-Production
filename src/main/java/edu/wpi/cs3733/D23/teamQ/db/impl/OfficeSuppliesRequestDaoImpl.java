...
public class OfficeSuppliesRequestDaoImpl implements GenDao<OfficeSuppliesRequest, Integer> {
    ...
    public boolean deleteRow(Integer requestID) {
        try (Connection connection = GenDao.connect(); PreparedStatement st = connection.prepareStatement("DELETE FROM \"officeSuppliesRequest\" WHERE \"requestID\" = ?")) {
            ;
    ...
    public boolean addRow(OfficeSuppliesRequest request) {
        try (Connection conn = GenDao.connect(); PreparedStatement stmt = conn.prepareStatement("INSERT INTO \"officeSuppliesRequest\"(requester, progress, assignee, \"specialInstructions\", \"item\", \"quantity\", \"roomNum\") VALUES (?, ?, ?, ?, ?, ?, ?)")) {
    ...
    public boolean populate() {
        try {
            ...
            while (rst.next()) {
                officeSuppliesRequests.add(new OfficeSuppliesRequest(rst.getInt("requestID"), rst.getString("requester"), rst.getInt("progress"), rst.getString("assignee"), nodeTable.retrieveRow(rst.getInt("node")), rst.getString("specialInstructions"), rst.getString("item"), rst.getInt("quantity")));
            }
            conn.close();
            stm.close();
        } catch (Exception e) {
