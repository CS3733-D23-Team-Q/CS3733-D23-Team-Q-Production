...
public class PathfindingController2 {
  ...
  public void addButtons(String f) {
    ...
    if (previousStart != null
        && !previousStart.equals("")
        && startSelect.getItems().contains(previousStart)) {
      startSelect.selectItem(previousStart);
    }
    if (previousEnd != null
        && !previousEnd.equals("")
        && endSelect.getItems().contains(previousEnd)) {
      endSelect.selectItem(previousEnd);
    }
      ...
      if (defaultsl != null) {
        if (n.getLocation().equals(defaultsl)) {
          if (highlightedNodesp.size() == 0) {
            ...
            if (startSelect.getSelectedIndex() == -1) {
  ...
  public List<Line> addLines(
      List<Pair<Node, Boolean>> path, List<Integer> indexes) { // List<Node> path
  ...
  public List<Line> drawLinesf(Node start, Node target, String floor)
    ...
    if (fpath.size() > 0) {
      for (int i = fpath.size() - 1; i >= 0; i--) {
        Node fn = fpath.get(i).getKey();
        String nodetype = fn.getLocation().getNodeType();
        if (nodetype.equals("ELEV") || nodetype.equals("STAI")) {
          for (int j = path.size() - 1; j > 0; j--) {
            Node n = path.get(j);
            Node next = path.get(j - 1);
            if (fn.equals(n)) {
              if (!fn.getFloor().equals(next.getFloor())) {
                indexes.add(i);
              }
            }
          }
  ...
  public void startSelected() throws IOException {
    ...
    if (startSelect.getSelectedIndex() != -1) { // lname != null && !lname.equals("")
  ...
  public void endSelected() throws IOException {
    ...
    if (endSelect.getSelectedIndex() != -1) { // lname != null && !lname.equals("")
